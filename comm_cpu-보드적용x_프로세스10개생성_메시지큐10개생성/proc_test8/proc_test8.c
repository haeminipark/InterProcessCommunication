...
//		printf("[7] tx_thread_function\r\n");
        ...
        if (msgsnd(MsgQId[MSGQ_8], &message, sizeof(message), IPC_NOWAIT) < 0) 
        {
            perror("msgsnd");
            exit(EXIT_FAILURE);
        }

		printf("proc8_tMsgQ[%02u]: ", MSGQ_8);
		for(int i=0; i<MSG_TEXT_MAX; i++)
        {
...
//		printf("[7] rx_thread_function\r\n");
        ...
        if (msgrcv(MsgQId[MSGQ_9], &message, sizeof(message), message.type, IPC_NOWAIT) < 0) 
        {
            if (errno == ENOMSG)    // msgrcv: No message of desired type 에러방지
            {
                printf("proc8_rMsgQ[%02u]: No message.\n", MSGQ_9);
            }
            else
            {
                perror("msgrcv");
...
int main(void)
{
    ...
    {
        perror("[PROC_TEST8] tx_thread_function");
        exit(EXIT_FAILURE);
    }
    else
    {
        printf("[PROC_TEST8] Create send_threads\r\n");
    }
	
    if (pthread_create(&recv_threads, NULL, rx_thread_function, NULL) != 0) 
    {
        perror("[PROC_TEST8] rx_thread_function");
        exit(EXIT_FAILURE);
    }
    else
    {
        printf("[PROC_TEST8] Create receive_threads\r\n");
    }
