...
void *tx_thread_function(void *arg)
{
	...
	while(1)
	{
//		printf("[3] tx_thread_function\r\n");
		pthread_mutex_lock(&send_mutex);

        if (msgsnd(MsgQId[MSGQ_3], &message, sizeof(message), IPC_NOWAIT) < 0) 
        {
            perror("msgsnd");
            exit(EXIT_FAILURE);
        }

		printf("proc3_tMsgQ[%02u]: ", MSGQ_3);
		for(int i=0; i<MSG_TEXT_MAX; i++)
        {
...
void *rx_thread_function(void *arg)
{
	...
	while(1)
	{
//		printf("[3] rx_thread_function\r\n");
		pthread_mutex_lock(&recv_mutex);

        if (msgrcv(MsgQId[MSGQ_2], &message, sizeof(message), message.type, IPC_NOWAIT) < 0) 
        {
            if (errno == ENOMSG)    // msgrcv: No message of desired type 에러방지
            {
                printf("proc3_rMsgQ[%02u]: No message.\n", MSGQ_2);
            }
            else
            {
                perror("msgrcv");
...
int main(void)
{
    ...
    {
        perror("[PROC_TEST3] tx_thread_function");
        exit(EXIT_FAILURE);
    }
    else
    {
        printf("[PROC_TEST3] Create send_threads\r\n");
    }
	
    if (pthread_create(&recv_threads, NULL, rx_thread_function, NULL) != 0) 
    {
        perror("[PROC_TEST3] rx_thread_function");
        exit(EXIT_FAILURE);
    }
    else
    {
        printf("[PROC_TEST3] Create receive_threads\r\n");
    }
