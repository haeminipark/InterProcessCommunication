...
//		printf("[1] tx_thread_function\r\n");
        ...
        if (msgsnd(MsgQId[MSGQ_1], &message, sizeof(message), IPC_NOWAIT) < 0) 
        {
            perror("msgsnd");
            exit(EXIT_FAILURE);
        }

		printf("proc0_tMsgQ[%02u]: ", MSGQ_1);
		for(int i=0; i<MSG_TEXT_MAX; i++)
        {
...
//		printf("[1] rx_thread_function\r\n");
        ...
        if (msgrcv(MsgQId[MSGQ_0], &message, sizeof(message), message.type, IPC_NOWAIT) < 0) 
        {
            if (errno == ENOMSG)    // msgrcv: No message of desired type 에러방지
            {
                printf("proc1_rMsgQ[%02u]: No message.\n", MSGQ_0);
            }
            else
            {
                perror("msgrcv");
...
int main(void)
{
    ...
    {
        perror("[PROC_TEST1] tx_thread_function");
        exit(EXIT_FAILURE);
    }
    else
    {
        printf("[PROC_TEST1] Create send_threads\r\n");
    }
	
    if (pthread_create(&recv_threads, NULL, rx_thread_function, NULL) != 0) 
    {
        perror("[PROC_TEST1] rx_thread_function");
        exit(EXIT_FAILURE);
    }
    else
    {
        printf("[PROC_TEST1] Create receive_threads\r\n");
    }
