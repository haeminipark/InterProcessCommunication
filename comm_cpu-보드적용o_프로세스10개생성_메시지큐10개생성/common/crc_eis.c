...
typedef struct        {
        uchar        i9 : 8;         /* bit8 ~ bit15 를 byte 로  */
        uchar        i8 : 1;         /* bit0 를 의미             */
        uchar        i7 : 1;
        uchar        i6 : 1;
        uchar        i5 : 1;
        uchar        i4 : 1;
...
typedef struct      {               /* crc table을 만드는 데 사용   */
        uchar        b16 : 1;         /* bit0 를 의미                 */
        uchar        b15 : 1;
        uchar        b14 : 1;
        uchar        b13 : 1;
        uchar        b12 : 1;
...
void  CrcTableCreate2(void)
{
    ...
    BITS8_BYTE  *index_ptr;      /* index 를 가르킬 pointer      */
    BITS16_UINT *word_ptr;       /* word 를 가르킬 pointer       */
...
ushort  CalcCrc (char *Buf, int Len)
{
    ...
    for (i = 0, crc = 0; i < Len; i++) /* 3: CRC-L, CRC-H, ETX */
    {
        ...
        crc = (crc >> 8) ^ crc2_table[index];  /* CRC 갱신             */
    }
