...
int main(void)
{
    ...
    {
        perror("[proc_test5] send_thread_function");
        exit(EXIT_FAILURE);
    }
    else
    {
        printf("[proc_test5] send_thread_function\r\n");
    }
	
    if (pthread_create(&recv_threads, NULL, recv_thread_function, NULL) != 0) 
    {
        perror("[proc_test5] recv_thread_function");
        exit(EXIT_FAILURE);
    }
    else
    {
